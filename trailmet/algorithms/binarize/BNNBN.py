...
class BNNBN():
    def __init__(self, model, dataloaders, **kwargs):
        ...
        self.name = "_".join([self.dataset, f"{self.epochs}", f"{self.learning_rate}", datetime.now().strftime("%b-%d_%H:%M:%S")])
        ...
        # logger.info(f"Experiment Arguments: {vars(self)}")

        # task = Task.init(
        #             project_name=f"Trailmet BNNBN",
        #             task_name=self.name,
    ...
    def compress_model(self):
        ...
        logger.info("Student Model: {}".format(self.model_student))
...
#             model_teacher = nn.DataParallel(model_teacher).cuda()
        ...
        if self.resume:
            ...
            if os.path.exists(checkpoint_tar):
                ...
                logger.info('Loading from the checkpoint {}'.format(checkpoint_tar))
                ...
                logger.info('Loaded checkpoint {} epoch = {}'.format(checkpoint_tar, ['epoch']))
            else:
        ...
        df_data=np.array([epochs_list, train_top1_acc_list, train_top5_acc_list, val_top1_acc_list, val_top5_acc_list]).T
        df = pd.DataFrame(df_data,columns = ['Epochs', 'Train Top1', 'Train Top5', 'Validation Top1', 'Validation Top5'])
    ...
    def train_kd(self, epoch, train_loader, model_student, model_teacher, criterion, optimizer, scheduler):
        ...
        epoch_iterator = tqdm(train_loader,
                              desc="Training KD in BNNBN Epoch [X] (X / X Steps) (batch time=X.Xs) (data time=X.Xs) (loss=X.X) (top1=X.X) (top5=X.X)",
        ...
        for i, (images, target) in enumerate(epoch_iterator):
            ...
            epoch_iterator.set_description(
                    "Training KD in BNNBN Epoch [%d] (%d / %d Steps) (batch time=%2.5fs) (data time=%2.5fs) (loss=%2.5f) (top1=%2.5f) (top5=%2.5f)" % (epoch, i, len(train_loader), batch_time.val, data_time.val, losses.val, top1.val, top5.val)
                )
            
            logger.info("Training KD in BNNBN Epoch [%d] (%d / %d Steps) (batch time=%2.5fs) (data time=%2.5fs) (loss=%2.5f) (top1=%2.5f) (top5=%2.5f)" % (epoch, i, len(train_loader), batch_time.val, data_time.val, losses.val, top1.val, top5.val))

            if self.wandb_monitor:
                wandb.log({'train_loss_KD': losses.val, 'train_top1_acc_KD': top1.val, 'train_top5_acc_KD': top5.val})
    ...
    def train(self, epoch, train_loader, model_student, criterion, optimizer, scheduler):
        ...
        epoch_iterator = tqdm(train_loader,
                              desc="Training in BNNBN Epoch [X] (X / X Steps) (batch time=X.Xs) (data time=X.Xs) (loss=X.X) (top1=X.X) (top5=X.X)",
                              bar_format="{l_bar}{r_bar}",
                              dynamic_ncols=True,
                              disable=False)
        ...
        for i, (images, target) in enumerate(epoch_iterator):
            ...
            epoch_iterator.set_description(
                    "Training Epoch [%d] (%d / %d Steps) (batch time=%2.5fs) (data time=%2.5fs) (loss=%2.5f) (top1=%2.5f) (top5=%2.5f)" % (epoch, i, len(train_loader), batch_time.val, data_time.val, losses.val, top1.val, top5.val)
            )

            logger.info("Training Epoch [%d] (%d / %d Steps) (batch time=%2.5fs) (data time=%2.5fs) (loss=%2.5f) (top1=%2.5f) (top5=%2.5f)" % (epoch, i, len(train_loader), batch_time.val, data_time.val, losses.val, top1.val, top5.val))

            if self.wandb_monitor:
                wandb.log({'train_loss': losses.val, 'train_top1_acc': top1.val, 'train_top5_acc': top5.val})
    ...
    def validate(self, epoch, val_loader, model, criterion):
        ...
        epoch_iterator = tqdm(val_loader,
                              desc="Validating in BNNBN Epoch [X] (X / X Steps) (batch time=X.Xs) (loss=X.X) (top1=X.X) (top5=X.X)",
                              bar_format="{l_bar}{r_bar}",
                              dynamic_ncols=True,
                              disable=False)
        ...
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(epoch_iterator):
                ...
                # if i%50 == 0:
                #     progress.display(i)
                epoch_iterator.set_description("Validating Epoch [%d] (%d / %d Steps) (batch time=%2.5fs) (loss=%2.5f) (top1=%2.5f) (top5=%2.5f)" % (epoch, i, len(val_loader), batch_time.val, losses.val, top1.val, top5.val))

                logger.info("Validating Epoch [%d] (%d / %d Steps) (batch time=%2.5fs) (loss=%2.5f) (top1=%2.5f) (top5=%2.5f)" % (epoch, i, len(val_loader), batch_time.val, losses.val, top1.val, top5.val))

                if self.wandb_monitor:
                    wandb.log({'val_loss': losses.val, 'val_top1_acc': top1.val, 'val_top5_acc': top5.val})
