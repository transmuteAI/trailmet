...
class BNNBN:
    ...
    def compress_model(self):
        ...
        if self.loss_type == "kd":
            ...
            if not classes_in_teacher == self.num_classes:
                ...
                print(
                    "* loading pretrained teacher weight from {}".format(
                        self.teacher_weight
                    )
                )
                pretrain_teacher = torch.load(self.teacher_weight, map_location="cpu")[
                    "state_dict"
                ]
        ...
        criterion_smooth = CrossEntropyLabelSmooth(
            self.num_classes, self.label_smooth
        ).cuda()
        ...
        if self.resume:
            ...
            if os.path.exists(checkpoint_tar):
                ...
                logger.info(
                    "Loaded checkpoint {} epoch = {}".format(checkpoint_tar, ["epoch"])
                )
            else:
    ...
    def train(
        self, epoch, train_loader, model_student, criterion, optimizer, scheduler
    ):
        batch_time = AverageMeter("Time", ":6.3f")
        data_time = AverageMeter("Data", ":6.3f")
        losses = AverageMeter("Loss", ":.4e")
