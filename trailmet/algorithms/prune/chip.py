...
class Chip(BasePruning):
    ...
    def get_feature_hook(self, module, input, output):
        os.makedirs(
            "conv_feature_map/" + self.arch + "_repeat%d" % (self.repeat), exist_ok=True
        )
        np.save(
            "conv_feature_map/"
            + self.arch
            + "_repeat%d" % (self.repeat)
            + "/conv_feature_map_"
            + str(self.conv_index)
            + ".npy",
            output.cpu().numpy(),
    ...
    def ci_score(self, path_conv):
        ...
        conv_reshape = conv_output.reshape(
            conv_output.shape[0], conv_output.shape[1], -1
        )
        ...
        for i in range(conv_reshape.shape[0]):
            for j in range(conv_reshape.shape[1]):
                r1_norm[i, j] = self.reduced_1_row_norm(
    ...
    def mean_repeat_ci(self, repeat, num_layers):
        ...
        for j in range(num_layers):
            ...
            repeat_ci_mean = []
            for i in tqdm(range(repeat), desc=f"Repeat CI for layer {j}", total=repeat):
                index = j * repeat + i + 1
                # add
    ...
    def load_resnet_model(self, model, oristate_dict):
        ...
        if orifilter_num != currentfilter_num:
            ...
            select_index = np.argsort(ci)[
                orifilter_num - currentfilter_num :
            ]  # preserved filter id
            select_index.sort()

            for index_i, i in enumerate(select_index):
                state_dict[name_base + conv_weight_name][index_i] = oristate_dict[
                    conv_weight_name
                ][i]
                for bn_part in bn_part_name:
                    state_dict[name_base + "bn1" + bn_part][index_i] = oristate_dict[
                        "bn1" + bn_part
                    ][i]
            ...
            for k in range(num):
                ...
                for l in range(iter):
                    ...
                    if orifilter_num != currentfilter_num:
                        ...
                        for bn_part in bn_part_name:
                            state_dict[name_base + bn_name + bn_part] = oristate_dict[
                                bn_name + bn_part
                            ]
                    ...
                    state_dict[
                        name_base + bn_name + ".num_batches_tracked"
                    ] = oristate_dict[bn_name + ".num_batches_tracked"]
                    cnt += 1
        ...
        for name, module in model.named_modules():
            ...
            if isinstance(module, nn.Conv2d):
                ...
                state_dict[name_base + name + ".weight"] = oristate_dict[
                    name + ".weight"
                ]
    ...
    def compress_model(self):
        ...
        for i in range(4):
            ...
            for j in tqdm(
                range(model.num_blocks[i]), desc=f"Block {i}", total=model.num_blocks[i]
            ):
    ...
    def train(self, epoch, train_loader, model, criterion, optimizer, scaler=None):
        ...
        epoch_iterator = tqdm(
            train_loader,
            desc="Training network Epoch [X] (X / X Steps) (batch time=X.Xs) (data time=X.Xs) (loss=X.X) (top1=X.X) (top5=X.X)",
    ...
    def validate(self, epoch, val_loader, model, criterion):
        ...
        epoch_iterator = tqdm(
            val_loader,
            desc="Validating network Epoch [X] (X / X Steps) (batch time=X.Xs) (loss=X.X) (top1=X.X) (top5=X.X)",
            bar_format="{l_bar}{r_bar}",
            dynamic_ncols=True,
            disable=False,
        )
