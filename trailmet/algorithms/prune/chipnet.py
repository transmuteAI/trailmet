...
from trailmet.algorithms.prune.prune_convert import convert_cifar_resnet
...
class ChipNet(BasePruning):
    ...
    def compress_model(self):
        ...
        if 'HARD_PRUNE' in self.kwargs:
            self.log_name = self.log_name + '_hard_pruned'
            print('preparing model for hard pruning')
            self.hard_prune(self.model)
            self.prepare_model_for_compression()
            self.model.to(self.device)
            self.prune(self.model, self.dataloaders, **self.kwargs['HARD_PRUNE'])
    ...
    def hard_prune(self, model):
        """function to hard prune a pretrained model using chipnet method"""
        print("Testing Hard-Pruned")  
        model = convert_cifar_resnet(model, insert_identity_modules=False)
