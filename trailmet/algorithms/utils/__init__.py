...
class LogTracker(object):
    ...
    def plot_heatmap(self, name, out_path, show_ticks=False):
        ...
        n_class = len(value[0])
        fig, ax = plt.subplots(
            figsize=[0.1 * len(step), n_class / 5]
        )  # /5 is set manually
...
class Logger(object):
    ...
    def __init__(self, args):
        ...
        self.log_printer = LogPrinter(
            self.logtxt, self.ExpID, self.args.debug or self.args.screen_print
        )  # for all txt logging
    ...
    def set_up_dir(self):
        ...
        self.script_hist = open(
            ".script_history", "a+"
        )  # save local script history, for convenience of check
    ...
    def print(self, step):
        ...
        if k:  # only when there is sth to print, print
            values += " (step = %d)" % step
            if (
                step % (self.args.print_interval * 10) == 0
    ...
    def __send_to_exp_hub(self):
        ...
        if self.SERVER in CONFIDENTIAL_SERVERS:
            today_remote = (
                "huwang@137.203.141.202:/homes/huwang/Projects/ExpLogs/%s" % today_exp
            )
        else:
            today_remote = (
                "wanghuan@155.33.198.138:/home/wanghuan/Projects/ExpLogs/%s" % today_exp
            )
