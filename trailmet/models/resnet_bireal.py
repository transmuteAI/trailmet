...
class HardBinaryConv(nn.Module):
    ...
    def forward(self, x):
        ...
        scaling_factor = torch.mean(torch.mean(torch.mean(torch.abs(real_weights),dim=3,keepdim=True),dim=2,keepdim=True),dim=1,keepdim=True)
...
class BinaryBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, binarise=True):
...
class BirealNet(ResNet):
    def __init__(self, block, layers, num_fp=0, width=1, num_classes=1000, produce_vectors=False, init_weights=True, insize=32):
        super(BirealNet, self).__init__(block, layers, width=1, num_classes=num_classes, produce_vectors=False, init_weights=True, insize=32)
        del self.prev_module
        self.num_fp = num_fp
        self.count_fp = 0
        self.inplanes = 64
        self.layer1 = self._make_layer1(block, 64 * width, layers[0])
        self.layer2 = self._make_layer1(block, 128 * width, layers[1], stride=2)
        self.layer3 = self._make_layer1(block, 256 * width, layers[2], stride=2)
        self.layer4 = self._make_layer1(block, 512 * width, layers[3], stride=2)
    ...
    def _make_layer1(self, block, planes, blocks, stride=1):
        ...
        if (self.count_fp<self.num_fp):
            ...
            layers = []
            layers.append(block(self.inplanes, planes, stride, downsample, binarise=False))
            self.inplanes = planes * block.expansion
            ...
            layers = []
            layers.append(block(self.inplanes, planes, stride, downsample, binarise=True))
            self.inplanes = planes * block.expansion
            for _ in range(1, blocks):
...
def make_birealnet18(num_classes, insize, num_fp=0):
    model = BirealNet(BinaryBasicBlock, [4, 4, 4, 4], num_classes=num_classes, insize=insize, num_fp=num_fp)
    return model


def make_birealnet34(num_classes, insize, num_fp=0):
    model = BirealNet(BinaryBasicBlock, [6, 8, 12, 6], num_classes=num_classes, insize=insize, num_fp=num_fp)
    return model


def make_birealnet50(num_classes, insize, num_fp=0):
    model = BirealNet(BinaryBasicBlock,[3, 4, 6, 3], num_classes=num_classes, insize=insize, num_fp=num_fp)
    return model
