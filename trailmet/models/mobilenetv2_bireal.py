...
class Block(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, expansion, stride, binary=True):
        ...
        if not self.binary:
            ...
            self.conv1 = HardBinaryConv(in_planes, planes, kernel_size=1, stride=1, padding=0)
            self.bn1 = nn.BatchNorm2d(planes)
            self.conv2 = HardBinaryConv(planes, planes, kernel_size=3, stride=stride, padding=1)
            self.bn2 = nn.BatchNorm2d(planes)
            self.conv3 = HardBinaryConv(planes, out_planes, kernel_size=1, stride=1, padding=0)
            self.bn3 = nn.BatchNorm2d(out_planes)
...
class MobileNetv2(BaseModel):
    ...
    def __init__(self, num_classes=10, num_fp=0):
        ...
        self.conv2 = nn.Conv2d(320, 1280, kernel_size=1, stride=2, padding=0, bias=False)
        ...
        for expansion, out_planes, num_blocks, stride in self.cfg:
            ...
            if self.curr_fp<self.num_fp:
                for stride in strides:
                    layers.append(Block(in_planes, out_planes, expansion, stride, binary=False))
                    in_planes = out_planes
                    self.curr_fp += 1
            else:
                for stride in strides:
                    layers.append(Block(in_planes, out_planes, expansion, stride, binary=True))
                    in_planes = out_planes
